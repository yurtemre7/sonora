...
class SonoraAppRouter {
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (context, state) =>
                InfoSettingsScreen(onResetApp: onResetApp),
          ),
          GoRoute(
  ...
  void _openNowPlayingSheet(BuildContext context) {
    ...
    // useRootNavigator: false pushes the sheet onto the shell's Navigator.
    // This means the shell's navigator stack becomes:
    //   [home → (settings|artist|album|playlist) → now_playing_sheet]
    // so Android's back gesture naturally dismisses the sheet first before
    // popping any underlying nested route.
