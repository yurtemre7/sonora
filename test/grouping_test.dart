...
void main() {
  test('buildAlbumGroups and buildArtistGroups derive stable snapshots', () {
    ...
    expect(albums.map((album) => album.name).toList(), [
      'Album One',
      'Album Two',
    ]);
  ...
  test(
    'buildAlbumGroups splits albums with the same name by different artists',
    () {
      var songs = [
        Song(
          id: 1,
      ...
      expect(
        albums.any((a) => a.name == 'Greatest Hits' && a.artist == 'Queen'),
        isTrue,
      );
      expect(
        albums.any(
          (a) => a.name == 'Greatest Hits' && a.artist == 'Guns N Roses',
        ),
        isTrue,
      );
