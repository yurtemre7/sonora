...
void main() {
  ...
  group('Massive Playlist & Library Simulation (500, 1000, 2000 Songs)', () {
    List<Song> generateSongs(int count) {
      return List.generate(count, (index) {
        ...
        return Song(
          ...
          filePath:
              '/mock/storage/music/Artist $artistId/Album $albumId/track_$id.flac',
    ...
    test(
      'Simulate 500-song playlist creation, JSON serialization, and grouping',
      () {
    ...
    test(
      'Simulate 2000-song playlist scaling, sort benchmarks, and filtering',
      () {
        ...
        var searchResults = songs
            .where(
              (s) =>
                  s.titleLower.contains(query.toLowerCase()) ||
                  s.artistLower.contains(query.toLowerCase()),
            )
            .toList();
  ...
  group('Playlist Cover Cropping & Downscaling Tests', () {
    test(
      'PlaylistImageUtils crops and resizes image to low-res JPEG file',
      () async {
        var sourcePng = File(
          '${Directory.current.path}/test/music/emre/artist.png',
        );
  ...
  group('UpdateService Rate-Limiting & Network Robustness Tests', () {
    test(
      'UpdateService handles network failures silently without crashing',
      () async {
        var result = await UpdateService.checkForUpdate();
        expect(result, isNotNull);
        expect(
          result.isRateLimited || result.hasError || result.update == null,
          isTrue,
        );
      },
    );
  ...
  group('CustomScrollbar & Widget Behavior Tests', () {
    testWidgets(
      'CustomScrollbar renders child and responds to drag notifications',
