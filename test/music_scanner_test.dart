...
void main() {
  ...
  group('MusicScanner & Metadata Extraction Tests (test/music target)', () {
    test('Directory structure exists for scanner tests', () {
      expect(musicDir.existsSync(), isTrue, reason: 'test/music directory should exist');
      expect(artistDir.existsSync(), isTrue, reason: 'emre artist folder should exist');
      expect(audioDir.existsSync(), isTrue, reason: 'emre/audio folder should exist');
      expect(audioMp3.existsSync(), isTrue, reason: 'audio.mp3 should exist');
      expect(audioCoverMp3.existsSync(), isTrue, reason: 'audio_cover.mp3 should exist');
    ...
    test('MusicScanner importFromFolder scans audio files and extracts metadata correctly', () async {
      ...
      expect(songs, isNotEmpty, reason: 'Scanner should discover audio files in test/music');
      expect(songs.length, equals(2));

      var songPlain = songs.firstWhere((s) => s.filePath == audioMp3.path);
      var songWithCover = songs.firstWhere((s) => s.filePath == audioCoverMp3.path);
      ...
      expect(artistImages.keys.any((k) => k.contains('emre') || k.contains(songPlain.artist.toLowerCase())), isTrue, reason: 'Artist emre image should be detected');
      var artistImagePath = artistImages['emre'] ?? artistImages[songPlain.artist.toLowerCase()] ?? artistImages.values.firstWhere((v) => v.contains('artist.png'));
    ...
    test('Grouping functions construct AlbumGroup and ArtistGroup with extracted songs', () async {
      ...
      var emreArtist = artists.firstWhere((a) => a.name.toLowerCase().contains('emre') || a.name == songs.first.artist);
      expect(emreArtist.localImagePath, equals(artistPng.path), reason: 'ArtistGroup should map artist.png from artist directory');
      expect(emreArtist.songs.length, equals(2));
    });
  });

  group('Widget Tests using Scanned Music Data', () {
    testWidgets('SongTile renders title, artist, and trailing menu', (WidgetTester tester) async {
      var sampleSong = Song(
        id: 1,
        title: 'Test Track',
        artist: 'Emre',
    ...
    testWidgets('ArtistAvatar renders real scanned artist cover image when localImagePath is detected', (WidgetTester tester) async {
    ...
    testWidgets('ArtistAvatar falls back cleanly when no local cover image exists', (WidgetTester tester) async {
      ...
      expect(find.byIcon(Icons.person_rounded), findsOneWidget, reason: 'Fallback icon should render when no cover image is available');
    });
  });
}
